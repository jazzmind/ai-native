...
const mdComponents: Components = {
  ...
  code: ({ className, children }) => {
    ...
    if (isBlock) {
      return <code className="block bg-[#0d0d0d] rounded-lg p-3 my-2 text-xs font-mono overflow-x-auto whitespace-pre">{children}</code>;
    }
    return <code className="bg-[#1a1a2e] px-1.5 py-0.5 rounded text-xs font-mono text-blue-300">{children}</code>;
  },
  pre: ({ children }) => <pre className="my-2">{children}</pre>,
  blockquote: ({ children }) => <blockquote className="border-l-2 border-[var(--accent)] pl-3 my-2 text-[var(--text-muted)]">{children}</blockquote>,
  ...
  th: ({ children }) => <th className="text-left px-2 py-1.5 font-semibold text-[var(--text-muted)]">{children}</th>,
  td: ({ children }) => <td className="px-2 py-1.5 border-b border-[var(--border)]">{children}</td>,
};
...
export function TabbedAdvisorResponse({
  ...
  const displayContent = showSynthesis
    ? synthesis.content
    : activeAdvisor?.content || "";

  const displayStreaming = showSynthesis
    ? synthesis.isStreaming
    : activeAdvisor?.isStreaming || false;

  const displayActivity = activeAdvisor?.activity;

  const displayMessageId = showSynthesis
    ? synthesis.messageId
    : activeAdvisor?.messageId;

  const displayCoachKey = showSynthesis
    ? "synthesis"
    : activeAdvisor?.coachKey;
      ...
      <div className="max-w-[85%] w-full">
        ...
        <div className="flex items-center gap-0.5 mb-0.5 ml-1 overflow-x-auto scrollbar-none">
          ...
          {synthesis && (
            ...
            >
              ...
              <span>Synthesis</span>
              {synthesis.isStreaming && (
                <span className="text-yellow-400" style={{ animation: "pulse-dot 1.5s infinite" }}>●</span>
              )}
            </button>
          )}
          ...
          {advisorTabs.map((tab) => (
            ...
            >
              ...
              {tab.isLead && (
                <span className="text-[9px] bg-[var(--accent)]/20 text-[var(--accent)] px-1 rounded-full">lead</span>
              )}
              {tab.isStreaming ? (
                <span className="text-[var(--accent)]" style={{ animation: "pulse-dot 1.5s infinite" }}>●</span>
              ) : tab.content ? (
        ...
        >
          ...
          {displayContent ? (
            ...
            <span className="inline-block text-[var(--text-muted)]" style={{ animation: "pulse-dot 1.5s infinite" }}>
              ●●●
            </span>
          ) : null}
