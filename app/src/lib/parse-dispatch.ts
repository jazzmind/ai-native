import type { EaMemoryType } from './db/queries/ea-memory';
...
const VALID_MEMORY_TYPES: EaMemoryType[] = ['template', 'recurring_task', 'contact', 'preference', 'context'];
...
export function parseMemoryBlocks(content: string): ParsedMemoryBlock[] {
  ...
  while ((match = BLOCK_REGEX.exec(content)) !== null) {
    ...
    // For memory blocks the content after the key-value header is the body
    const lines = raw.trim().split('\n');
    ...
    if (!VALID_MEMORY_TYPES.includes(type)) continue;
...
export function stripEaBlocks(content: string): string {
  return content.replace(/:::(dispatch|memory|expert_request)\n[\s\S]*?:::/g, '').trim();
}
