import type { TaskType } from '../providers/storage-provider';
...
const VALID_TYPES: readonly TaskType[] = [
  'coaching_followup', 'reminder', 'deadline',
  'check_in', 'status_report_collection', 'ea_briefing',
];
  ...
  while ((match = TASK_REGEX.exec(content)) !== null) {
    ...
    for (const line of lines) {
      if (line.startsWith('type:')) {
        const val = line.slice(5).trim() as TaskType;
